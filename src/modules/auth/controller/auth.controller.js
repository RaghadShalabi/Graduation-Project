...
export const signUp = async (req, res, next) => {
    ...
    if (isVictim) {
        ...
        newUser = await victimModel.create({ name, email, city, password: hashPassword, isVictim });
    } else {
        const existingRescueTeam = await rescueTeamsModel.findOne({ email });
        if (existingRescueTeam) {
            return next(new Error("Email already in use", { cause: 409 }));
        }
        newUser = await rescueTeamsModel.create({ name, email, city, password: hashPassword });
    }
    ...
    const token = jwt.sign({ email: email }, process.env.SECRET_KEY_EMAIL, { expiresIn: '1h' });
...
export const signIn = async (req, res, next) => {
    ...
    }, process.env.SIGN_IN_SECRET_KEY, { expiresIn: '1h' });

    const refreshToken = jwt.sign({
        id: user._id,
        role: isVictim ? user.role /*"Victim"*/ : user.role /*"RescueTeam"*/
    }, process.env.SIGN_IN_SECRET_KEY, { expiresIn: 60 * 60 * 24 * 30 });
...
export const confirmEmail = async (req, res, next) => {
    const token = req.params.token;  // Retrieve the token from the URL parameter
