...
export const getAllVictims = async (req, res, next) => {
    ...
    let victims = await victimModel.find({ city: rescueTeam.city })
...
export const getSosVictims = async (req, res, next) => {
    ...
    const victims = await victimModel.find({ city: rescueTeam.city, rescueStatus: false, sosStatus: true }).
        select("name city location heartRate rescueStatus");

    // If no victims are found, return a message indicating this
    if (victims.length === 0) {
        return res.status(200).json({ message: "No unRescued victims found in this city" });
    }
...
export const updateVictimRescueStatus = async (req, res, next) => {
    ...
    // Find the victim
    const victim = await victimModel.findById(victimId).select("name city location heartRate rescueStatus -_id");
    if (!victim) {
    ...
    res.status(200).json({ message: "Rescue status updated successfully", victim });
}
...
export const viewMap = async (req, res, next) => {
    ...
    const victims = await victimModel.find({ city: rescueTeam.city, sosStatus: true });
...
export const deleteDeadVictims = async (req, res, next) => {
    ...
    const result = await victimModel.deleteMany({ city: rescueTeam.city, heartRate: 0 });
    ...
    return res.status(200).json({ message: "Dead victims deleted successfully", deletedCount: result.deletedCount });
};
...
export const updateRescueTeamInfo = async (req, res, next) => {
    ...
    return res.status(200).json({ message: "rescue team user information updated successfully", rescueTeam });
};
...
export const updatePassword = async (req, res, next) => {
    ...
    // Return a success response
    return res.status(201).json({ message: "The password has been updated successfully", rescueTeam });
};